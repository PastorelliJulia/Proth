...
Static Function fGeraExcel()
	...
	cQuery += " WHERE "+ CRLF
	cQuery += " F1_EMISSAO BETWEEN '" +DTOS(mv_par01)+ "' AND '" +DTOS(mv_par02)+ "'" + CRLF
	...
	aRec := U_QryArr(cQuery) 
		...
		for x := 1 to len(aRec)
		    oExcel:AddRow("REINF INSS","REINF INSS",{aRec[x][1],aRec[x][2],aRec[x][3],aRec[x][4],aRec[x][5],aRec[x][6],sTod(aRec[x][7]),aRec[x][8],aRec[x][9],aRec[x][10],aRec[x][11],aRec[x][12],aRec[x][13],aRec[x][14],aRec[x][15]})
	    next x
        oExcel:Activate()
